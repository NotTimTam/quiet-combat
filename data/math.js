class MathFunctions {
	...
	isOnScreen(thing, camera, canvas) {
		if (thing.x + thing.width < camera.x) {
			return false;
		}

		if (thing.y + thing.height < camera.y) {
			return false;
		}

		if (thing.x > camera.x + canvas.width) {
			return false;
		}

		if (thing.y > camera.y + canvas.height) {
			return false;
		}
	...
	isOnScreen = (object, camera, canvas) => {
		// console.log(object);
